...
# Configure environment variables with development defaults
# For production deployment, set these as Replit secrets
...
if [ "$DEBUG" = "True" ] && [ -n "$REPL_ID" ]; then
    echo "⚠️  NOTE: Running in development mode on Replit. For production, set DEBUG=False as a secret."
fi
